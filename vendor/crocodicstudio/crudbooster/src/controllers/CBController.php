...
class CBController extends Controller {
		...
		foreach($data['result'] as $row) {
			...
			foreach($columns_table as $col) {
		          ...
		          if(@$col['download']) {
                    ...
                    if($col['urlLeadQuote']) {
                        $result = DB::table('account')->where('id', $value)->first();
                        $fullname = '';
                        if (!empty($result->name) && !empty($result->lastname)) {
                            $fullname = $result->name.' '.$result->lastname;
                        }
                        elseif (!empty($result->name) && empty($result->lastname)) {
                        ...
                        $value = "<a href=".$col['urlLead'].'/crm/account/detail/'.$result->id.">$fullname</a>";
                    }
                    ...
                    if($col['email']) {
                        ...
                        $value = "<a href='http://18.222.4.15/crm/account/send-email/$id->id')>$value</a>";
                    }
                    ...
                    if($col['urlTemplate']) {
                        $result = DB::table('cms_email_templates')->where('id', $value)->first();
                        $value = "<a href=".$col['urlTemplate'].'/edit/'.$value.">$result->name</a>";
                    }

                    if($col['urlQuote']) {
                        $result = DB::table('user_trucks')->where('truck_name', $value)->first();
                        $value = "<a href=".$col['urlQuote'].'/edit/'.$result->id.">$result->truck_name</a>";
                    }

                    if($col['urlPhone']) {
                        $value = "<a href=tel:$value target='_blank'>$value</a>";
                    }

                    if($col['address']) {
                        $value = "<a href=\"javascript:void(0)\" onclick=\"showModalMapaddress()\" title=\"Click to view the map\">
                                    <i class=\"fa fa-map-marker\"></i> ".$value."
                                </a>
                            
                                <div id=\"googlemaps-modal-address\" class=\"modal\" tabindex=\"-1\" role=\"dialog\" aria-hidden=\"true\" style=\"display: none;\">
                                    <div class=\"modal-dialog modal-lg\" role=\"document\">
                                        <div class=\"modal-content\">
                                            ...
                                            <div class=\"modal-body\"><div align=\"center\">Sorry the map is not found !</div></div>
                                        </div><!-- /.modal-content -->
                                    </div><!-- /.modal-dialog -->
                                </div><!-- /.modal -->
	...
	public function getAdd(){
		...
		$module_trans =  trans("crudbooster.".CRUDBooster::getCurrentModule()->name);
		//$page_title      = trans("crudbooster.add_data_page_title",['module'=>CRUDBooster::getCurrentModule()->name]);
		$page_title      = trans("crudbooster.add_data_page_title",['module'=>$module_trans]);
	...
	public function getEdit($id){
		...
		$page_menu       = Route::getCurrentRoute()->getActionName();
		$page_title 	 = trans("crudbooster.edit_data_page_title",['module'=>trans("crudbooster.".CRUDBooster::getCurrentModule()->name),'name'=>$row->{$this->title_field}]);
		$command 		 = 'edit';
	...
	public function getDetail($id)	{
		...
		if(!CRUDBooster::isRead() && $this->global_privilege==FALSE || $this->button_detail==FALSE) {
			CRUDBooster::insertLog(trans("crudbooster.log_try_view",['name'=>$row->{$this->title_field},'module'=>trans("crudbooster.".CRUDBooster::getCurrentModule()->name)]));
			CRUDBooster::redirect(CRUDBooster::adminPath(),trans('crudbooster.denied_access'));
		}
		...
		$page_menu  = Route::getCurrentRoute()->getActionName();
		$page_title = trans("crudbooster.detail_data_page_title",['module'=>trans("crudbooster.".CRUDBooster::getCurrentModule()->name),'name'=>$row->{$this->title_field}]);
		$command    = 'detail';
