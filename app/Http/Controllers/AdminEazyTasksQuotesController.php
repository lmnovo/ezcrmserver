<?php namespace App\Http\Controllers;
	...
	class AdminEazyTasksQuotesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Task Type","name"=>"task_type_id","join"=>"eazy_task_type,id"];
			$this->col[] = ["label"=>"Orders Assigned","name"=>"orders_id","join"=>"orders,business_name"];
			...
			$this->form[] = ['label'=>'Orders Assigned','name'=>'orders_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'orders,business_name'];
			...
			//$this->form[] = ["label"=>"Orders Id","name"=>"orders_id","type"=>"select2","required"=>TRUE,"validation"=>"required|integer|min:0","datatable"=>"orders,business_name"];
			# OLD END FORM
	    ...
	    */
	    public function hook_after_add($id) {
            $tasks = DB::table('eazy_tasks_quotes')->where('id', $id)->first();

            CRUDBooster::redirect(CRUDBooster::adminpath("orders/detail/$tasks->orders_id"),trans("crudbooster.text_task_added"));
	    }
